...
class Visualizer(QObject):
    ...
    def transfer_line(self):
        ...
        for seg in range(self.num_seg):
            ...
            if seg == 3:
                trans_B[seg] = transB(self.beta[seg]-self.beta[0])
            if seg == 6:
                trans_B[seg] = transB(self.beta[seg]-self.beta[3])
            base_[seg] = [self.lm[seg] /self.alpha[seg] * (1 - cos(self.alpha[seg]))*10,
                    0,
                    self.lm[seg] /self.alpha[seg] * sin(self.alpha[seg])*10
                        ]
        ...
        '''for seg in range(1, self.num_seg):
            ...
            if self.alpha[seg-1] == 0:
                base = [
                    0,0,
                    self.lm[seg-1]*multi
                ]
            else:
                base = [self.lm[seg-1] / self.alpha[seg-1] * (1 - cos(self.alpha[seg-1]))*multi,
                          0,
                          self.lm[seg-1] / self.alpha[seg-1] * sin(self.alpha[seg-1])*multi
                          ]
            transform = transA_(self.alpha[seg-1], base).dot(transB_(beta[seg-1])).dot(transform)
    ...
    def inverse_kinematic(self, pts=[0,0,0], n=[0,0,0], a=[0,0,0], euler=0, model=1, input=[],length=2):
        '''def test_square(dst, x0, a, n): #a1 a2 a3 b1 b2 b3 r1 r2 r3
            def test_3(x):
                a1 = float(x[0])
                a2 = float(x[1])
                a3 = float(x[2])
                ...
                result = np.array([
                    -((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(a3) / a3 + lm1 * (
                        ...
                        b3 - b2) / a3) * cos(a1) + (
                                       ...
                                       a2) / a3 + lm1 * sin(a2) / a2) * sin(a1) + lm1 * (1 - cos(a1)) / a1) * cos(b1)-dst[0],
                    ((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(a3) / a3 + lm1 * (
                            1 - cos(a2)) / a2) * sin(b2 - b1) + lm1 * (1 - cos(a3)) * sin(b3 - b2) * cos(
                        b2 - b1) / a3) * cos(
                        b1) + (((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(
                        ...
                        b3 - b2) / a3) * cos(a1) + (
                                           -lm1 * (1 - cos(a3)) * sin(a2) * cos(b3 - b2) / a3 + lm1 * sin(a3) * cos(
                                       a2) / a3 + lm1 * sin(a2) / a2) * sin(a1) + lm1 * (1 - cos(a1)) / a1) * sin(b1)-dst[1],
                    - ((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(a3) / a3 + lm1 * (
                    ...
                    -(1 - cos(a1)) * ((1 - cos(a2)) * (1 - cos(a3)) * sin(b2) * sin(b3) * cos(b2) * cos(b3) + (
                        ...
                        b3)) + ((1 - cos(a3)) * sin(a2) * sin(b2) * sin(b3) * cos(b3) - (
                            (1 - cos(a2)) * (-sin(b2) ** 2 - cos(b2) ** 2) + 1) * sin(a3) * cos(b3) - (
                                        -(1 - cos(a3)) * cos(b3) ** 2 + 1) * sin(a2) * cos(b2)) * sin(a1) * sin(b1) - n[1],
                    ((1 - cos(a1)) * (-sin(b1) ** 2 - cos(b1) ** 2) + 1) * (
            ...
            def test_31(x):
                ...
                result = np.array([
                    -((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(a3) / a3 + lm1 * (
                        ...
                        b3 - b2) / a3) * cos(a1) + (
                                           -lm1 * (1 - cos(a3)) * sin(a2) * cos(b3 - b2) / a3 + lm1 * sin(a3) * cos(
                                       a2) / a3 + lm1 * sin(a2) / a2) * sin(a1) + lm1 * (1 - cos(a1)) / a1) * cos(b1),
                    ((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(a3) / a3 + lm1 * (
                            1 - cos(a2)) / a2) * sin(b2 - b1) + lm1 * (1 - cos(a3)) * sin(b3 - b2) * cos(
                        b2 - b1) / a3) * cos(
                        b1) + (((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(
                        ...
                        b3 - b2) / a3) * cos(a1) + (
                                           -lm1 * (1 - cos(a3)) * sin(a2) * cos(b3 - b2) / a3 + lm1 * sin(a3) * cos(
                                       a2) / a3 + lm1 * sin(a2) / a2) * sin(a1) + lm1 * (1 - cos(a1)) / a1) * sin(b1),
                    - ((lm1 * (1 - cos(a3)) * cos(a2) * cos(b3 - b2) / a3 + lm1 * sin(a2) * sin(a3) / a3 + lm1 * (
                            1 - cos(a2)) / a2) * cos(b2 - b1) - lm1 * (1 - cos(a3)) * sin(b2 - b1) * sin(
                        b3 - b2) / a3) * sin(
                        a1) + (-lm1 * (1 - cos(a3)) * sin(a2) * cos(b3 - b2) / a3 + lm1 * sin(a3) * cos(
                        a2) / a3 + lm1 * sin(a2) / a2) * cos(a1) + lm1 * sin(a1) / a1
                    ,
                    - (1 - cos(a1)) * (-(1 - cos(a2)) * sin(a3) * sin(b2) * cos(b2) * cos(b3) + (
                            -(1 - cos(a2)) * sin(b2) ** 2 + 1) * sin(a3) * sin(b3) + (
                                               (1 - cos(a3)) * (-sin(b3) ** 2 - cos(b3) ** 2) + 1) * sin(a2) * sin(
            ...
            res = least_squares(test_3, x0_rosenbrock,
                              bounds=([0, 0, 0, -pi, -pi, -pi, 0.1, 0.1, 0.1], [pi, pi, pi, pi, pi, pi, np.inf, 1, 1]))
            ...
            n = [0,0,0]
            a = [0,0,0]
            print('pts:', test_3(result)[0:3])
            print('n:', test_3(result)[3:6])
            print('a:', test_3(result)[6:9])
            return result
        if model:
            # a1 a2 b1 b2 l1 l2
            self.dst_pos = pts
            ...
            pos_now = [self.alpha[0], self.alpha[3], self.alpha[6],
                       ...
                       self.lm[0]*3,1,1]
            #result = test_square(pts, pos_now, a, n)
        else:
            result = input'''
        ...
        '''except:
            print('inverse kinematic is failed')'''
    ...
    def animation(self):
        ...
        # ROS
            ...
            rospy.timer.Timer(40, self.update)
